...
from build import BUILD, BuildError, HARNESS, compile_profile, discover, discover_fixtures, launch_argv, load_profile, project_memory, write_readback  # noqa: E402
...
def approx_tokens(text: str | None) -> int:
    """문자수/4 어림. 실측(`jig budget`)이 아니라 **어느 몫인지 가리키는** 용도다."""
...
GLOBAL_SRC = HARNESS / "core" / "GLOBAL_CLAUDE.md"
GLOBAL_DST = Path.home() / ".claude" / "CLAUDE.md"
LANG_MARKERS = ("<!-- >>> jig language (generated by `jig lang <언어>`) >>> -->",
                "<!-- <<< jig language <<< -->")


def _lang_body(lang: str) -> str:
    return (
        "## Preferred Language\n\n"
        f"Respond in {lang} for every command and interaction: replies, explanations,"
        " end-of-turn summaries, and session/task summaries. Keep code, identifiers,"
        " file paths, and quoted output as-is. Only switch languages when the user"
        " explicitly asks for a different one."
    )


def current_lang() -> str | None:
    """템플릿에 지금 박혀 있는 언어. 블록에서 되읽는다 — 따로 저장하면 두 진실이 된다."""
    m = re.search(r"Respond in (.+?) for every command",
                  GLOBAL_SRC.read_text(encoding="utf-8"))
    return m.group(1) if m else None


def install_global() -> None:
    """`core/GLOBAL_CLAUDE.md` -> `~/.claude/CLAUDE.md`.

    이 파일은 모든 세션에 실린다 — 프로필 세션도 그렇다는 것을 쟀다
    (probe/results/memory-files.md). **병합이 아니라 덮어쓴다**: bootstrap 이 하나의
    알려진 상태를 만드는 것이 일이기 때문이다.

    설치 경로가 여기 하나뿐이어야 한다. bootstrap 이 따로 `cp` 를 하면 규칙이 두 벌이 된다.

    덮기 전에 **한 번만** 백업한다. 백업이 이미 있으면 남기지 않는다 — 지킬 가치가 있는
    것은 우리가 처음 덮은 그 파일(사람이 쓴 것)이고, 매번 갱신하면 두 번째 실행이
    그것을 우리 이전 설치본으로 갈아 치운다. tar 백업은 reset-and-setup.sh 경로에만
    있으므로, `jig lang` 이나 맨 bootstrap 으로 들어온 사람은 여기가 유일한 복구 지점이다.
    """
    GLOBAL_DST.parent.mkdir(parents=True, exist_ok=True)
    body = GLOBAL_SRC.read_text(encoding="utf-8")
    backup = GLOBAL_DST.with_suffix(".md.jigkit-backup")
    if GLOBAL_DST.is_file() and not backup.exists():
        old = GLOBAL_DST.read_bytes()
        if old != body.encode("utf-8"):
            backup.write_bytes(old)
            print(f"backed up {backup}")
    write_readback(GLOBAL_DST, body)


def cmd_lang(rest: list[str]) -> None:
    """응답 언어를 바꾸고 전역 지침을 다시 깐다.

    - `jig lang`             현재 값만 보여준다. **아무것도 쓰지 않는다.**
    - `jig lang <언어>`      템플릿을 고치고 다시 깐다
    - `jig lang --install`   언어는 그대로 두고 설치만 한다 (bootstrap 이 부르는 형태)

    조회가 조용히 파일을 쓰면 안 되므로 설치는 명시적으로 시킨다. 처음에는 무인자를
    "설치도 한다" 로 뒀는데, bootstrap 의 기본 경로가 그 무인자 호출이라 **전역 지침이
    아예 안 깔리면서 `ok` 만 찍혔다** — 조회와 설치를 한 이름에 겹쳐 놓은 대가였다.

    바꾸는 대상은 **템플릿**(`core/GLOBAL_CLAUDE.md`)이다. 설치본만 고치면 다음
    bootstrap 이 덮어써서 조용히 되돌아간다 — 진실은 저장소 쪽에 있어야 한다.
    """
    words = [r for r in rest if not r.startswith("-")]
    if not words and "--install" not in rest:
        print(f"{current_lang() or '(모름)'}   — 바꾸려면: jig lang <언어>")
        return
    if words:
        lang = " ".join(words)
        try:
            changed = mdblock.apply_block(GLOBAL_SRC, LANG_MARKERS, _lang_body(lang), write=True)
        except LookupError as e:
            die(str(e))
        print(f"{'updated' if changed else 'unchanged'} "
              f"{GLOBAL_SRC.relative_to(HARNESS)} — {lang}")
    install_global()
    print(f"installed {GLOBAL_DST}")
...
def cmd_doctor(names: list[str], project: Path) -> None:
    ...
    for n in names or (discover() + discover_fixtures()):
        ...
        # 프롬프트 크기가 이제 프로젝트에 따라 달라진다. 어느 몫인지 안 적으면
        # 하네스가 커진 것처럼 읽힌다.
        mem = approx_tokens(project_memory(project))

        print(f"ok   {n:<12} 프롬프트 ~{approx}토큰"
              f"{f' (프로젝트 지침 ~{mem}t 포함)' if mem else ''}"
              f" · 스킬 {len(skills)} (설명 ~{sk_tokens}t) · deny {len(deny)}건")
...
def cmd_budget(names: list[str], project: Path) -> None:
    ...
    failed = False
    # 편입된 프로젝트 지침도 기동 토큰에 들어간다. 상한은 **프로필의** 성질이므로,
    # 프로젝트 파일이 커서 넘친 것을 하네스 탓으로 읽히게 두면 귀속이 틀린다.
    mem = approx_tokens(project_memory(project))
    for n in names or discover():
        p = load_profile(n)
        ...
        if mem and not ok:
            print(f"     note 이 중 ~{mem:,}토큰은 이 프로젝트의 CLAUDE.md 편입분이다 —"
                  f" 상한은 프로필의 성질이므로 {project} 를 함께 본다")
    raise SystemExit(1 if failed else 0)
