...
한 도구가 표면을 둘 가지면 **id 를 둘로 쪼갠다**. `docker` 가 `library` 인 것은 빌드와 로그
읽기에서 이미 Bash 가 이기기 때문이고, 컨테이너 상태를 반복 조회해야 하는 드문 작업을 위해
`docker-mcp` 가 그 옆에 있다. `semgrep` 이 `gate` 인 것은 커밋을 막는 것이 목적이기 때문이고,
에이전트가 지적을 읽고 고쳐야 할 때를 위해 `semgrep-mcp` 가 있다. 기본값과 선택지가 각자의
근거와 함께 둘 다 기록된다.
