...
우회는 반대로 편향시킨다. 그리고 트리거와 우회는 **한 번의 `shlex` 토큰화**에서 함께
정한다 — 두 모델이 다르면 가장자리에서 갈라지기 때문이다. `git --git-dir .git commit`
을 못 읽는 정규식은 엉뚱한 명령에 붙은 우회 선언을 그대로 인정해 버린다. 판정 기준은
shell 의미론이다: `VAR=1 cmd` 는 그 단순 명령에만 붙으므로
`JIG_TOUCHED_BYPASS=1 echo hi; git commit` 은 **우회가 아니다.** 토큰화 자체가 실패하면
트리거는 넓게 두고 우회는 거절한다 — 둘 다 게이트가 켜진 채로 남는 방향이다.
